...
import { secutityDeviceServices } from '../services/usersSessions/secutityDeviceService';
import { add } from 'date-fns';
import { SETTINGS } from '../settings';
import { tokenService } from '../infrastructure/tokenService';
import { JwtPayload } from 'jsonwebtoken';

export const globalRequestLimitMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(req.cookies['refreshToken']){
        const userToken = await tokenService.validateRefreshToken(req.cookies['refreshToken']);
